...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\ulab-scribe-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("ulab-scribe-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("ulab-scribe-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/ulab-scribe-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("ulab-scribe-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/ulab-scribe-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("ulab-scribe-recordings")
    }
